...
const UsersListPage = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const res = await axios.get(
        `http://localhost:8000/api/admin/customers?page=${currentPage}&perPage=${usersPerPage}&search=${encodeURIComponent(
          searchTerm
        )}&sort=${sortOrder}`
      );
      console.log(res);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      Swal.fire(
        "Deleted!",
        "Selected customers have been deleted successfully.",
        "success"
      );
    } catch (error) {
