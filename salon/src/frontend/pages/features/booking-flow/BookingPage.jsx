...
export default function BookingPage() {
  ...
  const [bookingResponse, setBookingResponse] = useState(null);
  const [isLoading, setIsLoading] = useState(Boolean(id));
  const [isPrefillDone, setIsPrefillDone] = useState(false);
  ...
  useEffect(() => {
    ...
    setFormData((p) => ({
      ...p,
      name: authUser?.user?.name || p.name,
      email: authUser?.user?.email || p.email,
      phone: authUser?.user?.phone || p.phone,
      category_id: bookingData?.category?.id
        ? String(bookingData.category.id)
        : p.category_id,
      sub_category_id: bookingData?.subcategory?.id
        ? String(bookingData.subcategory.id)
        : p.sub_category_id,
    ...
    if (bookingData) setStep("calendar");
  ...
  const selectedSubCategory = subCategories.find(
    (s) => String(s.id) === String(formData.sub_category_id)
  );
